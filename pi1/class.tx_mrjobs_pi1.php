...
class tx_mrjobs_pi1 extends tslib_pibase {
	...
	private function displayJobOfferList() {
	    ...
	    while($row = $TYPO3_DB->sql_fetch_assoc($result)) {
	        ...
	        $content .= '<p><b>' . $this->pi_getLL('employer') . ':</b><br />' . $row['employer'] . '</p><p><b>' . $this->pi_getLL('workaddress') . ':</b><br /> ' . $row['workaddress'] . '<br />' . $this->pi_getLL('commune') . ': ' . $row['commune'] . '<br />' . $this->pi_getLL('canton') . ': ' . $row['canton'] . '</p>';
